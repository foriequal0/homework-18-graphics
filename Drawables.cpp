...
SweptSurface SweptSurface::create(const SweptSurfaceData &data) {
  ...
  case CurveType::CATMULL_ROM:
    cross_sections = interpolate_cross_sections(catmull_rom<Bezier, Eigen::Vector3f>, data.cross_sections);
    break;
  case CurveType::BSPLINE:cross_sections = interpolate_cross_sections(bspline<Eigen::Vector3f>, data.cross_sections);
    break;
